...
func _update(delta: float) -> void:
	...
	if agent:
		agent.velocity.x = move_toward(agent.velocity.x, 0, Global.BASE_DECELERATION * delta)
		agent.velocity.z = move_toward(agent.velocity.z, 0, Global.BASE_DECELERATION * delta)
