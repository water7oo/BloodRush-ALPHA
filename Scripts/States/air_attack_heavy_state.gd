...
func _process_attack(delta: float) -> void:
	...
	if Global.attackHeavyAir_cooldown_timer <= 0.0 and recovery_timer <= 0.0:
		buffered_input = false
		can_chain_attack = true
		if can_chain_attack && Input.is_action_just_pressed("attack_medium_1") && Input.is_action_just_pressed("attack_heavy_1") && !agent.is_on_floor():
			_exit_attack_state()
			agent.state_machine.dispatch(next_attack_state)
		else:
			_exit_attack_state()
...
func _start_attack() -> void:
	...
	next_attack_state = "to_airSLamAttack"  # Light -> Medium

func _on_attack_box_area_entered(area):
	if isHit:
		return
	if area.has_method("takeDamageEnemy") && area.current_health > 0:
		area.takeDamageEnemy(PlayerAttackManager.AirHeavyAttackDamage)
		Global.combo_hits.append({
	"enemy": area,
	"damage": PlayerAttackManager.AirHeavyAttackDamage,
	"attack_type": "attackHeavyAir",
